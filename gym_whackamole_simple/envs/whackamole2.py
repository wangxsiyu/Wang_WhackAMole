...
class WhackAMole2(gym.Env):
    ...
    def __init__(self, render_mode = None, window_size = (512, 512), render_fps = 20, n_frame_per_episode = 500, version = "full"):
        ...
        low = np.array([-vMAX, -vMAX])#.astype(np.float32)
        high = np.array([vMAX, vMAX])#.astype(np.float32)
    ...
    def step(self, action):
        ...
        r1 = self.my_observation_space["gaze"].step(action)
        r2 = self.my_observation_space["mole"].step(self.my_observation_space["gaze"].obs(), 0)
        reward = r1 + r2
